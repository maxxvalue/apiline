<?php
$table = "poke";
$access_token = 'Jg8VkHLUeNtxDLCVaL9dHWP4CCTSGN0/2sU9TPsSZrP/NxrDu1FaH7SnumXvQ2MpH+Z57NqWWuSgn1aqudSOd5oDzUsRFTid7uam0GcP7lDGH6aZZh3PBAlMiy6HT5A0biCvZlC8Khre1aak/8DGfAdB04t89/1O/w1cDnyilFU=';
...
if (!is_null($events['events'])) {
	foreach ($events['events'] as $event) {
		...
		if (in_array($groupId, ['Cf8f0e0a09af3a8cd72b8ccb04ec3bdc3'])) { //test

			//ตั้งห้องรีพอร์ต
			$reportRoom = 'Cf8f0e0a09af3a8cd72b8ccb04ec3bdc3';
...
🚩ถอนเงินแจ้ง พพ. ก่อนปิดรอบ 10 นาที/หลังปิด15 นาที';
				...
				if ($type == 'sticker' && $packageId == '2000004' && $stickerId == '48963') {
					if ($data['status'] == 'close') {
						$sentText = resultRest($data['game'], $data['bigLap']);
						$bigLap = $data['bigLap'];
						...
						$data = json_encode($data);
						update($table, 'content', $data, 'id', 1);
						send($access_token, $reportRoom, $sentText);
					}
					else {
						$replyText = 'โปรดเช็คสถานะ';
					}
				...
				if (in_array($text, ['@ok', '@OK', '@Ok'])) {
					...
					if (clear($replyText) != '') {
						...
						$replyText = result();
						$data['lap'] = ($data['lap'] == 1 ? 2 : $data['lap']);
						$data['status'] = 'close';
						$data['result'] = '';
						$data = json_encode($data);
						update($table, 'content', $data, 'id', 1);
				...
				if ($text[0] == '$'){
					if ($data['status'] == 'close') {
						...
						$replyText = 'ยังไม่ถึงรอบการเติมเงิน/ถอนเงิน';
					}
				}
			}
//---------------------------------------คำสั่งของ admin--------------------------------------------//
