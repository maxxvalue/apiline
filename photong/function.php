...
$playRoom = 'Cf4c29160ba12d941541aaf928cd38d9f';
...
//เก็บข้อมูลรายวัน
function reportAll($table, $day, $week, $month, $year, $conclude, $deposite, $withdraw) {
	global $con;
	$sql = "INSERT INTO $table (day, week, month, year, conclude, deposite, withdraw)
	VALUES ('$day', '$week', '$month', '$year', '$conclude', '$deposite', '$withdraw')";
	if (!$con->query($sql)) {
		echo $con->error;
		$return = false;
	}
...
function resultClose($game) {
	...
	$month = date('n');
	$year = date('Y') + 543;
	reportAll('reportAllPoke', $day, $week, $month, $year, $profit, $deposite, $withdraw);
	return $reply;
}
